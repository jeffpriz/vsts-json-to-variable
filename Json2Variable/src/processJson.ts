    ...
    while (dataQueue.length > 0) {
      ...
      if (thisDataItem != undefined) {
        if (thisDataItem.DataObj != undefined) {
          tl.debug(
            "Popped: " +
              thisDataItem.DataObj.toString() +
              " | " +
              thisDataItem.PrefixChain
          );
...
async function ProcessSingleNode(
  dataQueue: dataItem.DataItem[],
  thisJson: any,
  thisDataItem: dataItem.DataItem,
  shouldPrefix: boolean
) {
  if (thisJson != undefined) {
    ...
    var isArray: boolean = await processArrayNode(
      dataQueue,
      thisJson,
      thisDataItem,
      shouldPrefix
    ...
    if (!isArray) {
      isComplexObject = await processComplexObject(
        dataQueue,
        thisJson,
        thisDataItem,
        shouldPrefix
...
export async function processComplexObject(
  dataQueue: dataItem.DataItem[],
  thisJson: any,
  thisDataItem: dataItem.DataItem,
  shouldPrefix: boolean
